...
CURRENT_PATH = "simplecGAN_MNIST_useLabel"
...
def Generator(n_samples, conditions, noise=None):
    ...
    # conditional input: digit in image
    conds = tf.reshape(conditions, [n_samples, 1]) 
    conds = tf.matmul(conds, tf.ones([1, 128]))
    conds = tf.reshape(conds, [-1, 128]) 
    # for now just concat the inputs: noise as second dim of cond image 
    output = tf.concat([noise, conds], 1)  # to: (BATCH_SIZE,256)
    output = tf.reshape(output, [n_samples, 256]) 
    output = linearStep('Gen.Input', 256, 4*4*4*DIM, output)
...
def Discriminator(inputs, conditions):
    ...
    # conditional input: digit on image, fill whole matrix with it
    conds = tf.reshape(conditions, [BATCH_SIZE, 1]) 
    conds = tf.matmul(conds, tf.ones([1, IM_DIM*IM_DIM]))
    conds = tf.reshape(conds, [-1, 1, IM_DIM, IM_DIM])   
...
condition_data = tf.placeholder(tf.int64, shape=[BATCH_SIZE]) # conditional input for both G and D
cond_data = tf.cast(condition_data, tf.float32) 
...
fixed_cond_data = tf.cast(fixed_labels, tf.float32) 
...
if(CONTINUE):
    ...
    fixed_noise = tf.Variable(tf.random_normal(shape=[BATCH_SIZE, 128]), name='noise') #var inst of const: var is saved, dtype float implicit  # shape for additional channel
...
with tf.Session() as session:
    ...
    for iteration in range(START_ITER, ITERS):  # START_ITER: 0 or from last checkpoint
        ...
        if iteration > 0:
            ...
            _ = session.run(gen_train_op, feed_dict={cond_data: _labels})
        # Train duscriminator
