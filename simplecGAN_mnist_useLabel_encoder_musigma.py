...
ITERS = 300000 # How many generator iterations to train for # 200000 takes too long
...
CURRENT_PATH = "mnist_useLabel_encoder_musigma" #results/
...
def Encoder(inputs):
    ...
    # nn.BatchNorm2d(32),
    output = lays.conv2d(output, 2*DIM, kernel_size = 5, stride = 2, # 64
            data_format='NCHW', activation_fn=tf.nn.leaky_relu, 
            weights_initializer=tf.initializers.he_uniform(),
    ...
    encoded = lays.fully_connected(output, Z_DIM, activation_fn=None, 
                     weights_initializer=tf.initializers.glorot_uniform(), 
                     reuse = tf.AUTO_REUSE, scope = 'Enc.Images')
    mean = lays.fully_connected(output, Z_DIM, activation_fn=None, 
                     weights_initializer=tf.initializers.glorot_uniform(), 
                     reuse = tf.AUTO_REUSE, scope = 'Enc.Mean') 
    variance = lays.fully_connected(output, Z_DIM, activation_fn=None,
                     weights_initializer=tf.initializers.glorot_uniform(), 
                     reuse = tf.AUTO_REUSE, scope = 'Enc.Var') 
    return encoded, mean, variance

def sample_noise(shape, mean = 0.0, stddev = 1.0): # sample from Gaussian
    return tf.random_normal(shape, mean= mean, stddev = stddev)   

def sample_z(shape, mean, variance):
    stddev = tf.exp(0.5 * variance) # variance has to be positive
    noise = tf.random_normal(shape) # sample from N(0,1)
    return (noise * stddev) + mean  # scale & location
    
def Generator(n_samples, mean, variance, noise=None):    
    if noise is None:
        noise = sample_noise([n_samples, Z_DIM])
    # input: mean and var
    code = sample_z([n_samples, Z_DIM], mean, variance)
    # concat the inputs: z appended to noise
...
def Discriminator(inputs, conditions):
    ...
    output = tf.reshape(output, [-1, 4*4*4*DIM]) # adjust
    # conditional input: label of digit on image; given after conv; concat
...
enc_images, mean, variance = Encoder(real_data)

fake_data = Generator(BATCH_SIZE, mean, variance)
...
# VAE loss function combines reconstruction loss (e.g. Cross Entropy, MSE) with KL divergence. # TODO
#recon_loss = cross_entropy(fake_data, real_data)
#kl_loss = tf.reduce_mean(0.5 * tf.reduce_sum(tf.exp(logvar) + mean**2 - 1. - logvar, 1))
...
fixed_noise_samples = Generator(BATCH_SIZE, fixed_mean, fixed_var noise=fixed_noise) # Generator(n_samples,conds, noise)
