...
def Discriminator(inputs, conditions):	# input conds as well
    inputs = tf.reshape(inputs, [-1, 2, 32, 32])   # input flow --> dim is different: 2 instead of 3
...
fixed_cond_samples, fixed_cond_samples_flow = next(gen)  # shape: (batchsize, 3072) 
...
fixed_real_data_int = fixed_cond_samples_flow # TODO: check if uint8! change size to 32x32!
...
def generate_image(frame, true_dist):   # generates 64 (batch-size) samples next to each other in one image!
    ...
    # print(samples.shape)
    samples2show = np.append(samples, fixed_real_data_int) # show last flow next to generated samples.. is also np??
    lib.save_images.save_images(samples2show.reshape((2*BATCH_SIZE, 2, IM_DIM, IM_DIM)), 'samples_{}.jpg'.format(frame))
# does it work with 2 channel images? how to show? 
    ...
    for i in range(0, BATCH_SIZE):
        ...
        x = img_as_float(real)
        pred = np.reshape(samples[i] , (IM_DIM,IM_DIM,2)) 
        y = img_as_float(pred)
        mseval = mse(x, y)
        ssimval = ssim(x, y, multichannel = True)
...
with tf.Session() as session:
    ...
    for iteration in range(ITERS):
        ...
        # Train generator
        if iteration > 0:
            _data, _ = next(gen)  # shape: (batchsize, 6144) ##not 3072 anymore  # flow as second argument not needed
            _cond_data = _data[:,0:OUTPUT_DIM] # earlier frame as conditional data,
            # _real_data = _data[:,3072:] # last frame as real data for discriminator not needed
        ...
        for i in range(disc_iters):
            _data, _flow = next(gen)  # shape: (batchsize, 6144) ##not 3072 anymore  # flow as second argument
            # extract real and cond data
            _cond_data = _data[:,0:OUTPUT_DIM] # earlier frame as conditional data,
            _real_data = _flow #_data[:,OUTPUT_DIM:] # flow as real data for discriminator
