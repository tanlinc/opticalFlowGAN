...
def Generator(n_samples, noise=None):    
    if noise is None:
        ...
        noise = tf.reshape(noise, [n_samples, 1, 128]) 
...
test_gen = sintel.load_test_gen(BATCH_SIZE, (IM_DIM,IM_DIM,3))

# For generating samples: define fixed noise
fixed_samples, _ = next(test_gen)  # shape: (batchsize, 2*output_dim)
fixed_real_data_int = fixed_samples[:,OUTPUT_DIM:]  # next frame as real for disc, shape (64,3072)
if(CONTINUE):
    fixed_noise = tf.get_variable("noise", shape=[BATCH_SIZE, IM_DIM*IM_DIM]) # get noise from saved model
else:
    fixed_noise = tf.Variable(tf.random_normal(shape=[BATCH_SIZE, IM_DIM*IM_DIM]), name='noise') #var inst of const: var is saved, dtype float implicit
...
with tf.Session() as session:
    ...
    for iteration in range(START_ITER, ITERS):  # START_ITER: 0 or from last checkpoint
        ...
        for i in range(DISC_ITERS):
            ...
            _disc_cost, _ = session.run([disc_cost, disc_train_op], feed_dict={real_data: _real_data})
        ...
        if iteration % 100 == 99:
            ...
            _real_data = _data[:,OUTPUT_DIM:] # current frame for disc 
            _dev_disc_cost = session.run(disc_cost, feed_dict={real_data: _real_data})
