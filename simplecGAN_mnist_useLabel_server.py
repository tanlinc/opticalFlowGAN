...
CURRENT_PATH = "results/mnist_useLabel_onlylaterindisc_300000" #results/
restore_path = "/home/linkermann/opticalFlow/opticalFlowGAN/" + CURRENT_PATH + "/model.ckpt" # changed for server
...
def Generator(n_samples,conditions, noise=None):    
    ...
    output = lays.conv2d_transpose(output, 2*DIM, kernel_size= 5, stride = 2, 
            weights_initializer=tf.keras.initializers.he_uniform(),
...
fixed_labels = tf.reshape(tf.tile(tf.expand_dims(y_test_tf,axis=-1),[1,5]),[-1])
...
def generate_image(frame, true_dist):   # generates a batch of samples next to each other in one image!
    ...
    imsaver.save_images(samples_255.reshape((BATCH_SIZE, 1, IM_DIM, IM_DIM)), 'samples_{}.jpg'.format(frame))
    print("Iteration %d : \n" % frame)
...
with tf.Session() as session:
    ...
    for iteration in range(START_ITER, ITERS):  # START_ITER: 0 or from last checkpoint
        ...
        # Train generator
        if iteration > 0:
	    _data, _labels = mnist.train.next_batch(batch_size=BATCH_SIZE)
        ...
        if iteration % 100 == 99:
            dev_disc_costs = []
            _data, _labels = mnist.test.next_batch(batch_size=BATCH_SIZE)
